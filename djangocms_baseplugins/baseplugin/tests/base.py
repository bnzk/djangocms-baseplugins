...
class BasePluginTestCase(object):
    ...
    def setUp(self):
        ...
        abstract_models = importlib.import_module(
    ...
    def _reload_plugins_settings(self):
        ...
        if not self.plugin_path:
            print(  # noqa
                "not yet reloadable, add plugin_path to testcase for: {}".format(
                    self.plugin_class.__name__
                )
            )
        else:
            conf = importlib.import_module(
                "{}.{}".format(self.plugin_path, self.plugin_conf)
            )
            cms_plugins = importlib.import_module(
                "{}.cms_plugins".format(self.plugin_path)
            )
    ...
    def test_basic_admin_form_overrides(self):
        ...
        placeholder = Placeholder.objects.create(page=page, slot="test")
        settings_kwargs = {
            "{}_LAYOUT_CHOICES".format(self.plugin_settings_prefix): (
                ("0000-layout", "Nope"),
            ),
            "{}_COLOR_CHOICES".format(self.plugin_settings_prefix): (
                ("0000-color", "Nope"),
            ),
            "{}_BACKGROUND_CHOICES".format(self.plugin_settings_prefix): (
                ("0000-background", "Nope"),
            ),
            "{}_SIZE_CHOICES".format(self.plugin_settings_prefix): (
                ("77-size77", "Nope"),
            ),
            "{}_CUSTOM_CHOICES".format(self.plugin_settings_prefix): (
                ("99-custom99", "Nope"),
            ),
            "{}_DESIGN_FIELDS".format(self.plugin_settings_prefix): (
                "layout",
                "color",
                "background",
                "size",
    ...
    def test_plugin_bem_classes_applied(self):
        ...
        model_instance = add_plugin(placeholder, self.plugin_class, "en", **data)
        renderer = ContentRenderer(request=RequestFactory())
        html = renderer.render_plugin(model_instance, {})
        plugin_name = model_instance.__class__.__name__.lower()
        self.assertIn("plugin_{}".format(model_instance.pk), force_str(html))
        self.assertIn("plugin-{}".format(plugin_name), force_str(html))
        self.assertIn(
            "plugin-{}_{}".format(plugin_name, "layout-value"), force_str(html)
        )
        self.assertIn(
            "plugin-{}_layout-{}".format(plugin_name, "layout-value"), force_str(html)
        )
        self.assertIn(
            "plugin-{}_{}".format(plugin_name, "color-value"), force_str(html)
        )
        self.assertIn(
            "plugin-{}_{}".format(plugin_name, "background-value"), force_str(html)
        )
        self.assertIn("plugin-{}_{}".format(plugin_name, "size-value"), force_str(html))
        self.assertIn(
            "plugin-{}_size-{}".format(plugin_name, "size-value"), force_str(html)
        )
        self.assertIn(
            "plugin-{}_{}".format(plugin_name, "custom-value"), force_str(html)
        )
        self.assertIn(
            "plugin-{}_anchor-{}".format(plugin_name, "anchor-value"), force_str(html)
        )
    ...
    def test_translated_improperly_configured(self):
        ...
        #         "test_app.models.CustomAbstractBasePlugin"
        #     ),
        # }
        # with self.settings(**settings_kwargs):
        #     baseplugin_models = importlib.import_module(
        #         "djangocms_baseplugins.image.models"
        #     )
