...
def check_settings(prefix, conf, settings):
    ...
    for field_setting in ["CONTENT_FIELDS", "DESIGN_FIELDS", "ADVANCED_FIELDS"]:
        for field in flatten(getattr(conf, field_setting)):
            ...
            _check_one_setting(
                prefix, conf, settings, "HELP_TEXT_{}".format(field.upper())
            )
            _check_one_setting(
                prefix, conf, settings, "WIDGET_{}".format(field.upper())
            )
    ...
    if needs_check:
        if app_name not in modules:
            raise ImproperlyConfigured(
                ...
                "or you cannot translate plugins, "
                "or have a custom abstract baseplugin!".format(app_name)
            )
...
def sanitize_richtext(text):
    if defaults.LXML_CLEANER_CONFIG:
        ...
        if bleach:
            ...
            msg = "bleach not installed, (should be?), for sanitizing richtext content!"
            raise ImproperlyConfigured(msg)
    return text
...
def is_edit_mode(toolbar):
    if (
        getattr(toolbar, "edit_mode", None)
        or getattr(toolbar, "edit_mode_active", None)  # cms pre 3.6
        or getattr(toolbar, "build_mode", None)  # cms 3.6+
